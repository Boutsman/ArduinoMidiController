...
void readAnalog(int pin, int n)  
{
      ...
      if (val[n] != lastVal[n]) {
           ...
           midiMsg.msg.status = MIDI_COMMAND_CONTROL_CHANGE+chan[n];
           midiMsg.msg.data1    = note[n];
           midiMsg.msg.data2    = val[n];
...
void readDigital(int mux, int n)
{
  ...
  if(val[n] == 1){
     if(lastVal[n] == 0){
        midiMsg.msg.status = MIDI_COMMAND_NOTE_ON+chan[n];
        midiMsg.msg.data1    = note[n];  
        midiMsg.msg.data2    = 127;
  ...
  if(val[n] == 0){
     if(lastVal[n] == 1){
        midiMsg.msg.status = MIDI_COMMAND_NOTE_OF+chan[n];
        midiMsg.msg.data1    = note[n];  
        midiMsg.msg.data2    = 0;
...
{
  //Digital Values 1
  val[n] = digitalRead(digPin);

  if(val[n] == 1){
     if(lastVal[n] == 0){
       int tempInt = map(analogRead(anaPin), 0, 1023, 0, 127);
        midiMsg.msg.status = MIDI_COMMAND_NOTE_ON+chan[n];
        midiMsg.msg.data1    = note[n];  
        midiMsg.msg.data2    = tempInt;
...
void chanAfterTouch(int pin, int n){
   //Analoge Values
      ...
      if (val[n] != lastVal[n]) {
           ...
           //send control change on chan[n]
           midiMsg.msg.status = MIDI_CHANAFTERTOUCH+chan[n];
           midiMsg.msg.data1   = val[n];
...
void polyphonicAfterTouch(int pin, int n){
   //Analoge Values
      ...
      if (val[n] != lastVal[n]) {
           ...
           //send control change on chan[n]
           midiMsg.msg.status = MIDI_POLYAFTERTOUCH+chan[n];
           midiMsg.msg.data1   = note[n];
           midiMsg.msg.data2 = val[n];
